...
function printErrorMsg(msg) {
  console.log(
    colors.blue("[" + getCurrentDateTime() + "]"),
    colors.red("[ERROR]"),
    msg
  );
}
...
function login() {
  return Promise.try(function() {
    ...
    .then(function(response) {
      ...
      var csrfToken = $(
        '#main form[action="/auth/login/"] input[name="csrfmiddlewaretoken"]'
      ).val();
    ...
    .then(function(response) {
      ...
      var liveModels = $("#main div.content ul.list")
        ...
        .filter(function() {
          return (
            $(this)
              .find("div.details ul.sub-info li.cams")
              .text() != "offline"
          );
        })
        .map(function() {
      ...
      printDebugMsg(
        "Found these live followed models: " + liveModels.join(", ")
      );
...
function getCommandArguments(modelName) {
  return Promise.try(function() {
    ...
    .then(function(response) {
      ...
      var streamServer = streamData[1].split(",").map(function(line) {
      ...
      var passwordHash = script
        .match(/password: '([^']+)'/)[1]
        .replace("\\u003D", "="); // As of 2015-05-15, this is a PBKDF2-SHA256 hash of the user's password, with the iteration count and salt generously provided. I could replace the empty line below with a line to make bhttp send this hash to my own server, where I'd be able to crack it at my leisure, but as you can see, that line is empty, you're welcome. :)
        ...
        fs.stat(config.captureDirectory + "/" + filename, function(err, stats) {
          if (err) {
            if (err.code == "ENOENT") {
              ...
              printErrorMsg(
                "[" + colors.green(modelName) + "] " + err.toString()
              );
            }
            ...
            fs.rename(
              config.captureDirectory + "/" + filename,
              config.completeDirectory + "/" + filename,
              function(err) {
                if (err) {
                  printErrorMsg(
                    "[" + colors.green(modelName) + "] " + err.toString()
  ...
  return fs
    ...
    .then(function(stats) {
      ...
      if (stats.size - model.size > 0) {
        ...
        printErrorMsg(
          "[" + colors.green(model.modelName) + "] Process is dead"
        );
    ...
    .catch(function(err) {
      if (err.code == "ENOENT") {
        ...
        printErrorMsg(
          "[" + colors.green(model.modelName) + "] " + err.toString()
        );
      }
    });
}
